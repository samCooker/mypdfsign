...
import cn.com.chaochuang.writingpen.model.MotionElement;
        ...
        if(isEraser){
            ...
            switch (event.getActionMasked()) {
                case MotionEvent.ACTION_DOWN:
                    //记录down的控制点的信息
                    mLastPoint = new ControllerPoint(event.getX(), event.getY());
                    canvas.drawCircle(event.getX(),event.getY(),eraseWidth,erasePaint);
                    return true;
                case MotionEvent.ACTION_MOVE:
                    ControllerPoint curPoint2 = new ControllerPoint(event.getX(), event.getY());
                    canvas.drawLine(mLastPoint.x,mLastPoint.y,curPoint2.x,curPoint2.y,erasePaint);
                    canvas.drawCircle(event.getX(),event.getY(),eraseWidth,erasePaint);
                    mLastPoint = curPoint2;
                    return true;
                default:
                    break;
            }
